...
let rjust = function(s, width, fillchar=' ') {
    let result
    let sum = width - s.length
    if (sum > 0) {
        let zero = ''
...
var center = function(s, width, fillchar=' ') {
    ...
    result = zuoyou(left) + s + zuoyou(right)
    return result
}
...
var test_center = function() {
    ...
    ensure(center('guagua', 6) === 'guagua', 'center 测试 5')
}
// test_center()
